{if is_set($is_facetsearch)|not}
    ...
    {/if}
{else}
    <i id="map{$attribute.id}" 
       class="fa fa-map-marker fa-2x" 
       style="cursor: pointer;" 
       onclick="showModal({$attribute.id});" 
       data-latitude="{$attribute.content.latitude}" 
       data-longitude="{$attribute.content.longitude}" 
