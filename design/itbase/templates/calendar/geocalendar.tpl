                ...
                {if is_set( $calendarData.day_by_day[$day_id] )}
                    {if $calendarData.day_by_day[$day_id].count|gt(0)}            
                        ...
                        {foreach $calendarData.day_by_day[$day_id].events as $event max 4}
                            <li>
                                {def $label_color = 'label-success'}
                                {def $color = $event.node|attribute('color')}
                                
                                {if $color.content[0]|eq(0)}
                                    {set $label_color = 'label-danger'}
                                {elseif $color.content[0]|eq(1)}
                                    {set $label_color = 'label-success'}
                                {elseif $color.content[0]|eq(2)}
                                    {set $label_color = 'label-info'}
                                {elseif $color.content[0]|eq(3)}
                                    {set $label_color = 'label-warning'}
                                {/if}
                                <a class="has-tooltip label {$label_color}" href={$event.main_url_alias|ezurl()} title="{$event.name|wash()}" data-toggle="tooltip" data-placement="top">
                                    {$event.name|wash()}
                                </a>
                            </li>
                        {/foreach}
