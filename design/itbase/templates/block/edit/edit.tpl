                ...
                {if is_set( $custom_attribute_types[$custom_attrib] )}
                    {switch match = $custom_attribute_types[$custom_attrib]}
                        ...
                        {case match = 'state_select'}
                          ...
                          <input id="block-custom_attribute-{$block_id}-{$loop_count}" class="textfield block-control" type="text" name="ContentObjectAttribute_ezpage_block_custom_attribute_{$attribute.id}[{$zone_id}][{$block_id}][{$custom_attrib}]" value="{$block.custom_attributes[$custom_attrib]|wash()}" />
                        {/case}  
                        {case match = 'title_size'}
                            {def $cur_Value = $block.custom_attributes[$custom_attrib]}
                            
                            <select class="select_to_string" onchange="select_to_string(this,true)">                           
                            <option value="1" {if $cur_Value|eq('1')} selected  {/if} > H1 </option>
                            <option value="2" {if $cur_Value|eq('2')} selected  {/if} > H2 </option>
                            <option value="3" {if $cur_Value|eq('3')} selected  {/if} > H3 </option>
                            <option value="4" {if $cur_Value|eq('4')} selected  {/if} > H4 </option>
                            </select><input type="hidden" id="block-custom_attribute-{$block_id}-{$loop_count}" 
                                   class="textfield block-control-value-3" type="text" 
                                   name="ContentObjectAttribute_ezpage_block_custom_attribute_{$attribute.id}[{$zone_id}][{$block_id}][{$custom_attrib}]" 
                                   value="{$block.custom_attributes[$custom_attrib]|wash()}" />
                            {undef $cur_Value}
                        {/case}
                        {case match = 'select_true_false'}
                            {def $cur_Value = $block.custom_attributes[$custom_attrib]}
                            <select class="select_to_string" onchange="select_to_string(this,true)">
                                <option></option>
                                <option value="0" {if $cur_Value|eq('0')} selected  {/if}> NO </option>
                                <option value="1" {if $cur_Value|eq('1')} selected  {/if}> Si </option>                      
                            </select> 
                                <input type="hidden" id="block-custom_attribute-{$block_id}-{$loop_count}" 
                                   class="textfield block-control-value-2" type="text" 
                                   name="ContentObjectAttribute_ezpage_block_custom_attribute_{$attribute.id}[{$zone_id}][{$block_id}][{$custom_attrib}]" 
                                   value="{$block.custom_attributes[$custom_attrib]|wash()}" />
                            {undef $cur_Value}   
                        {/case}
