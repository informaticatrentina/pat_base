...
<form class="calendar-tools" method='GET' action={concat('calendar/view/', $node.node_id)|ezurl}>
    ...
    <input type='hidden' name="View" value="geocalendar" />
    ...
    <div class="navigation-calendar hidden-xs pull-right">
        <div class="btn-group">
          <button type="submit" name="PrevMonthCalendarButton" class="btn btn-default">
              <i class="fa fa-angle-left"></i>
          </button>
          <button type="submit" name="NextMonthCalendarButton" class="btn btn-default">
    ...
    <table summary="Calendario degli eventi" class="table-geocalendar">
    <thead>
    <tr>
        ...
        <td class="{if and( is_set( $calendarData.day_by_day[$day_id] ), $calendarData.day_by_day[$day_id].count|gt(0) )}events {/if}{if eq($counter, $temp_today)}ezagenda_selected {/if}{if and(eq($counter, $curr_today), eq($curr_month, $temp_month))}ezagenda_current {/if}{$css_col_class}">        
                ...
                <h3 style="font-size: 1em; margin:0;">
                    <span class="day-of-week">
                        {$dayofweeks[$dayofweek]|i18n("design/ocbootstrap/full/event_view_calendar")}
                    </span> 
                    <span class="num {if and(eq($counter, $curr_today), eq($curr_month, $temp_month))}label label-primary{/if}">
                        {$counter}
                    </span>
                </h3>
                    <br>
                ...
                {if is_set( $calendarData.day_by_day[$day_id] )}
                    {if $calendarData.day_by_day[$day_id].count|gt(0)}            
                        ...
                        {foreach $calendarData.day_by_day[$day_id].events as $event max 4}
                            <li>
                                {def $label_color = 'text-success'}
                                {def $__color = $event.node|attribute('color')}
                                
                                {if $__color.content[0]|eq(0)}
                                    {set $label_color = 'text-danger'}
                                {elseif $__color.content[0]|eq(1)}
                                    {set $label_color = 'text-success'}
                                {elseif $__color.content[0]|eq(2)}
                                    {set $label_color = 'text-info'}
                                {elseif $__color.content[0]|eq(3)}
                                    {set $label_color = 'text-warning'}
                                {/if}
                                <a class="has-tooltip {$label_color}" href={$event.main_url_alias|ezurl()} title="{$event.name|wash()}" data-toggle="tooltip" data-placement="top">
                                    {$event.name|wash()}
                                </a>
                                {undef $__color
                                       $label_color}
